...
* `stockflow` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `stockflow` - executable or bundle name on disk.
* `StockFlow` - display name shown in window titles and about app dialogs.
* `com.stockflow` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
* `StockFlow` - the name of the company.
